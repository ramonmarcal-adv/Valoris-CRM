  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(payload.error || t('toastAlreadyInAnotherAccount'));
        } else {
      ...
      toast.error(t('toastFailedReach'));
      setAccepting(false);
    }
  }, [token, t]);
      ...
      <Card className="w-full max-w-md border-border bg-card">
        <CardHeader className="items-center text-center">
          ...
          <CardTitle className="text-xl text-foreground">
            {t(`failReason.${peek.reason}.title`)}
          </CardTitle>
          <CardDescription className="text-muted-foreground">
            {t(`failReason.${peek.reason}.body`)}
          </CardDescription>
        </CardHeader>
