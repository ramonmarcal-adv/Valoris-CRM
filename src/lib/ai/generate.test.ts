...
describe('generateReply — Gemini', () => {
  it('calls the generateContent endpoint with the api key header and parses candidate text', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      okResponse({
    ...
    const res = await generateReply({
      config: config({ provider: 'gemini', model: 'gemini-test', apiKey: 'AIza-x' }),
      systemPrompt: 'sys',
      messages: [{ role: 'user', content: 'Hello' }],
    })
  ...
  it('throws on an empty candidate response', async () => {
    vi.stubGlobal(
      'fetch',
