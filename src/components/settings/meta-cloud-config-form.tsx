...
/**
 * Meta Cloud API credentials form — the original single-provider
 * WhatsApp Settings panel, unchanged behavior, extracted out of
 * `whatsapp-config.tsx` so that file could become a provider-tab shell
 * shared with `EvolutionConfigForm`. All state/handlers here are
 * scoped to `api_type='meta_cloud'` via the API routes they call.
 */
export function MetaCloudConfigForm() {
  const t = useTranslations('Settings.whatsapp');
  const supabase = createClient();
  // After multi-user, whatsapp_config is one-row-per-account (per
  // provider, post-048), not one-row-per-user. We pull `accountId`
  // straight off the auth context and key every read off it — so a
  // teammate who just joined an account sees the inviter's saved
  // config without having to re-enter anything.
  ...
  const fetchConfig = useCallback(async (acctId: string) => {
    setLoading(true);
    try {
      // Load form values from Supabase (shows what's in DB). Scoped to
      // meta_cloud: an account can now also have an Evolution row
      // (migration 048), and `.maybeSingle()` throws if both match.
      ...
      toast.error(t('toastConfigLoadFailed'));
    } finally {
      setLoading(false);
    }
  }, [supabase, t]);
  ...
  async function handleSave() {
    if (!phoneNumberId.trim()) {
      toast.error(t('toastPhoneNumberRequired'));
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error || t('toastSaveConfigFailed'));
        setSaving(false);
        return;
      }
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          t('toastRegistrationFailed', { error: data.registration_error }),
          { duration: 12000 },
        );
      ...
      toast.error(t('toastSaveConfigFailed'));
    } finally {
      setSaving(false);
    }
  }
