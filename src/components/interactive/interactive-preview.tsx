 ...
 * customer taps them on their own device). Its only 2 fallback strings
 * (shown before the agent has typed anything, or for a malformed
 * button/row) share the `Interactive.builder` namespace already owned
 * by interactive-builder.tsx, its only other consumer.
 */
    ...
    >
      <div className="px-3 py-2">
        ...
        <p className="whitespace-pre-wrap break-words text-sm">
          {payload.body || (
            <span className="text-muted-foreground">{t("previewEmptyBody")}</span>
          )}
        </p>
        {payload.footer ? (
      ...
      {payload.kind === "buttons" ? (
        ...
        >
          ...
          <span className="truncate">{payload.button_label || t("listButtonFallback")}</span>
        </button>
      )}
    </div>
  );
